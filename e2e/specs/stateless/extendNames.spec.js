...
describe('Extend Names', () => {
    ...
    it('should be able to register multiple names on the address page', () => {
      ...
      NAMES.forEach((name) => {
        cy.findByTestId(`name-item-${name}`).findByTestId('short-expiry-foreground').invoke('attr', 'data-timestamp').then(timestamp => {
      ...
      NAMES.forEach((name) => {
        cy.get(`@${name}`).then(timestamp => {
          ...
          cy.findByTestId(`name-item-${name}`).findByTestId('short-expiry-foreground').invoke('attr', 'data-timestamp').should('eq', newTimestamp.toString())
        })
      })
    })
    ...
    it('should extend a single name', () => {
      ...
      cy.findByTestId('expiry-label').invoke('attr', 'data-timestamp').then(timestamp => {
      ...
      cy.get('@timestamp').then(timestamp => {
        ...
        cy.findByTestId('expiry-label').invoke('attr', 'data-timestamp').should('eq', newTimestamp.toString())
      })
    })
